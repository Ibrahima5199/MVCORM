    ...
    <div class="container mt-5">
        <div class="row">
            ...
            <div class="col-md-4 col-xs-12">
                <div class="card">
                    <h5 class="card-header text-center text-white bg-dark">FORMULAIRE DE MODIFICATION DES FORMATIONS</h5>
                    <div class="card-body">
                        <form method="post" action="/mvcOrm2/Formation/update">
                            <div class="form-group">
                                <label>ID</label>
                                <input class="form-control" readonly type="text" value="<?= $data["formation"]->getId() ?>" name="id">
                            </div>
                            <div class="form-group">
                                <label>Nom</label>
                            ...
                            <div class="form-group">
                                <label>Duree</label>
                                <input class="form-control" type="number" value="<?= $data["lieu"]->getDuree() ?>" name="duree" required>
                            </div>
                            <div class="form-group">
                                <label>Date</label>
                                <input class="form-control" type="date" value="<?= $data["lieu"]->getDate() ?>" name="date" required>
                            </div>
                            <div class="form-group">
                                <label>Lieu</label>
                                <select class="form-control" name="idlieu">
                                    ...
                                    <?php foreach ($data["listeLieux"] as $lieux) { ?>
                                        <option <?php if ($lieux->getId() == $data["lieu"]->getId()) { ?> selected <?php } ?> value="<?= $lieux->getId() ?>"><?= $lieux->getNom() ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="form-group">
